<div
    ...
    x-init="({loading, swiftpayQrOrders, links} = await fetchSwiftQrOrders('{{route('swiftpay_qr_query_orders.index')}}', {...search, ...getDateFromAndTo()}))"
>
    <div class="pt-5">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form
                @submit.prevent="{loading, swiftpayQrOrders, links} = await fetchSwiftQrOrders('{{route('swiftpay_qr_query_orders.index')}}', {...search, ...getDateFromAndTo()})"
            >
                <div class="flex mb-3">
                        ...
                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">
                            ...
                            <li>
                                <button
                                    type="button"
                                    class="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                                    x-on:click="search = {field: 'id', label: 'Id'}"
                                >
                                    ID
                                </button>
                            </li>
    ...
    <x-table>
            ...
            <template x-for="(link, index) in links">
                <div class="inline-block">
                    <template x-if="index.includes('prev')">
                        <x-pagination-link
                            x-on:click="{loading, swiftpayQrOrders, links} = await fetchSwiftQrOrders(link, {...search, ...getDateFromAndTo()})"
                            label="Prev"/>
                    </template>
        ...
        <x-slot:body>
            <template x-for="swiftpayQrOrder in swiftpayQrOrders"
                      :key="swiftpayQrOrder.id">
                <tr>
                    <template x-for="field in fields" :key="field">
...
<x-slot:script>
    <script>
        ...
        async function fetchSwiftQrOrders(url, params = {}) {
            ...
            return fetch(url)
                ...
                .then(response => ({
                    swiftpayQrOrders: response.data,
